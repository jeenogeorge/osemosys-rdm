...
exp_id = 'Integrated'  # THIS IS AN INPUT
...
desi_u_data = {}
general_u_keys = ['drivers', 'drivers_max','drivers_min','drivers_max_norm', 'drivers_min_norm',
    ...
    for p in range(len(unique_periods)):  # "unique_periods" is from above
        ...
        for o in sd_outcome_unique:
            ...
            for this_d in drivers_all_intermediary_local + drivers_all_uncertainty_local:
                ...
                high_thlds_list_no_norm = \
                    [thr_val[n] for n in range(len(the_dir)) if 'high' in the_dir[n]]
                ...
                if len(high_thlds_list) != 0:
                    ...
                    high_thlds_avg_no_norm = \
                        sum(high_thlds_list_no_norm)/len(high_thlds_list_no_norm)
                    high_thlds_avg_pre_no_norm = 0
                    if high_thlds_avg < 0.01:
                        high_thlds_avg_pre_no_norm = deepcopy(high_thlds_avg_no_norm)
                        high_thlds_avg_no_norm = 99
                ...
                low_thlds_list_no_norm = \
                    [thr_val[n] for n in range(len(the_dir)) if 'low' in the_dir[n]]
                if len(low_thlds_list) != 0:
                    low_thlds_avg = \
                        sum(low_thlds_list)/len(low_thlds_list)
                    low_thlds_avg_pre = 0
                    
                    low_thlds_avg_no_norm = \
                        sum(low_thlds_list_no_norm)/len(low_thlds_list_no_norm)
                    low_thlds_avg_pre_no_norm = 0
                    if low_thlds_avg > 0.99:
                        low_thlds_avg_pre_no_norm = deepcopy(low_thlds_avg_no_norm)
                        low_thlds_avg_no_norm = 99
                ...
                if case in [1, 2, 3]:  # definitive
                    ...
                    if 'high' in direction_content:
                        #print('ENTRA 1')
                        final_min = high_thlds_avg_no_norm
                        final_max = max(high_thlds_list_no_norm)
                        final_min_norm = high_thlds_avg
                        final_max_norm = 1
                    elif 'low' in direction_content:
                        #print('ENTRA 2')
                        final_min = min(low_thlds_list_no_norm)
                        final_max = low_thlds_avg_no_norm
                        final_min_norm = 0
                    ...
                    if (final_min_norm > 1 or final_max_norm > 1):
                        ...
                        if final_min_norm == 99:
                            final_min = high_thlds_avg_pre_no_norm
                            final_min_norm = high_thlds_avg_pre
                        elif final_max_norm == 99:
                            final_max = low_thlds_avg_pre_no_norm
                    ...
                    final_min = (low_thlds_avg_no_norm + high_thlds_avg_no_norm)/2
                    final_max = final_min
                ...
                if outcome_type == 'desirable':
                    ...
                    desi_u_data[o]['drivers_min'][per].append(final_min)
                    desi_u_data[o]['drivers_max'][per].append(final_max)
                    ...
                    risk_u_data[o]['drivers_min'][per].append(final_min)
                    risk_u_data[o]['drivers_max'][per].append(final_max)
        ...
        for this_d in drivers_all_intermediary + drivers_all_uncertainty:
            ...
            if outcome_type == 'desirable':
                ...
                desi_u_data['All']['drivers_min'][per].append(final_min)
                desi_u_data['All']['drivers_max'][per].append(final_max)
                ...
                risk_u_data['All']['drivers_min'][per].append(final_min)
                risk_u_data['All']['drivers_max'][per].append(final_max)
...
for metric in list_u_data_metrics:
    for p in range(len(unique_periods)):  # "unique_periods" is from above
        ...
        for n in range(len(print_driver_list)):
            ...
            print_max = desi_u_data[metric]['drivers_max'][per][n]
            print_min = desi_u_data[metric]['drivers_min'][per][n]
            ...
            long_drivers_max.append(print_max)
            long_drivers_min.append(print_min)
...
for metric in list_u_data_metrics:
    for p in range(len(unique_periods)):  # "unique_periods" is from above
        ...
        for n in range(len(print_driver_list)):
            ...
            print_max = risk_u_data[metric]['drivers_max'][per][n]
            print_min = risk_u_data[metric]['drivers_min'][per][n]
...
end_result_dict = {'Outcome_Type':long_outcome_type,
                   ...
                   'Min':long_drivers_min,
                   'Max':long_drivers_max,
